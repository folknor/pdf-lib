...
const _bin = {
    ...
    readUint: (buff, p) => buff[p] * (256 * 256 * 256) +
...
function _decompress(out, dd, w, h) {
    ...
    const interlace = out.interlace;
    if (interlace === 0)
        result = _filterZero(result, out, 0, w, h);
    else if (interlace === 1)
        result = _readInterlace(result, out);
    return result;
}
function _inflate(data, buff) {
    const out = inflateRaw(new Uint8Array(data.buffer, data.byteOffset + 2, data.length - 6), buff);
    return out;
}
    ...
    for (let y = 0; y < h; y++) {
        ...
        if (type === 0)
            ...
            for (; x < bpl; x++)
                data[i + x] =
                    data[di + x] +
                        _paeth(data[i + x - bpp], data[i + x - bpl], data[i + x - bpp - bpl]);
        }
    }
    return data;
}
