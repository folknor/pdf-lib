...
import { CustomFontEmbedder, CustomFontSubsetEmbedder, decodePDFRawStream, JpegEmbedder, PageEmbeddingMismatchedContextError, PDFArray, PDFCatalog, PDFContext, PDFDict, PDFHexString, PDFName, PDFObjectCopier, PDFPageEmbedder, PDFPageLeaf, PDFPageTree, PDFParser, PDFStream, PDFStreamWriter, PDFString, PDFWriter, PngEmbedder, StandardFontEmbedder, UnexpectedObjectTypeError, } from '../core/index.js';
...
import { EncryptedPDFCopyError, EncryptedPDFError, FontkitNotRegisteredError, ForeignPageError, InvalidFontkitError, RemovePageFromEmptyDocumentError, } from './errors.js';
...
export default class PDFDocument {
    ...
    static async load(pdf, options = {}) {
        ...
        if (!!context.lookup(context.trailerInfo.Encrypt) &&
            ...
            const decryptedContext = await PDFParser.forBytesWithOptions(bytes, parseSpeed, throwOnInvalidObject, warnOnInvalidObjects, capNumbers, new CipherTransformFactory(encryptDict, fileIds.get(0).asBytes(), password), forIncrementalUpdate).parseDocument();
     ...
     */
    async copyPages(srcDoc, indices, options) {
     ...
     */
    registerCopiedFormFields(pages, renameTransform) {
        ...
        for (const page of pages) {
            ...
            for (let i = 0; i < annots.size(); i++) {
                ...
                if (processedFields.has(rootDict))
                    continue;
     ...
     */
    renameFieldHierarchy(fieldDict, transform, _processedFields) {
        ...
        const newPartialName = lastDot >= 0 ? newFullName.slice(lastDot + 1) : newFullName;
     ...
     */
    getFieldFullName(fieldDict) {
        const parts = [];
        let currentDict = fieldDict;
        while (currentDict) {
            ...
            const parentRef = currentDict.get(PDFName.of('Parent'));
            if (parentRef) {
                const parentDict = this.context.lookup(parentRef);
    ...
    getRawAttachments() {
        ...
        for (let idx = 0, len = EFNames.size(); idx < len; idx += 2) {
            ...
            if (!fileSpec)
                continue; // Skip invalid/null entries
            rawAttachments.push({
                fileName,
                fileSpec,
        ...
        return rawAttachments.flatMap(({ fileName, fileSpec, specRef }) => {
            ...
            const mimeType = subtype instanceof PDFName
                ...
                : subtype instanceof PDFString
                    ? subtype.decodeText()
                    : undefined;
            const paramsDict = embeddedFileDict.lookupMaybe(PDFName.of('Params'), PDFDict);
            let creationDate;
            ...
            return [
                {
                    ...
                    mimeType: mimeType?.replace(/#([0-9A-Fa-f]{2})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16))),
    ...
    async embedSvg(svg) {
        ...
        const findImages = (element) => {
            if (element.tagName === 'image')
                ...
                return element.childNodes.flatMap((child) => child.nodeType === NodeType.ELEMENT_NODE ? findImages(child) : []);
            }
        };
    ...
    async save(options = {}) {
        ...
        if (useObjectStreams) {
            if (incrementalUpdate) {
                const increment = await PDFStreamWriter.forContextWithSnapshot(this.context, objectsPerTick, this.context.snapshot, true, 50, compress).serializeToBuffer();
            ...
            return PDFStreamWriter.forContext(this.context, objectsPerTick, true, 50, compress).serializeToBuffer();
        }
        else {
            if (incrementalUpdate) {
                const writer = PDFWriter.forContextWithSnapshot(this.context, objectsPerTick, this.context.snapshot, compress);
    ...
    async saveIncremental(snapshot, options = {}) {
        ...
        if (saveOptions.useObjectStreams) {
            return PDFStreamWriter.forContextWithSnapshot(this.context, objectsPerTick, snapshot, true, 50, compress).serializeToBuffer();
        }
        else {
            const writer = PDFWriter.forContextWithSnapshot(this.context, objectsPerTick, snapshot, compress);
            writer.fillXrefGaps = fillXrefGaps;
            return writer.serializeToBuffer();
        }
    }
    ...
    async commit(options = {}) {
        ...
        const incrementalBytes = await this.saveIncremental(this.context.snapshot, options);
    ...
    updateInfoDict() {
        ...
        if (!info.get(PDFName.of('Producer')))
