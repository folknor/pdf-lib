...
// prettier-ignore
export const setFillingColor = (color) => color.type === ColorTypes.Grayscale
...
// prettier-ignore
export const setStrokingColor = (color) => color.type === ColorTypes.Grayscale
    ...
    : color.type === ColorTypes.RGB
        ...
        : color.type === ColorTypes.CMYK
            ...
            : error(`Invalid color: ${JSON.stringify(color)}`);
// prettier-ignore
export const componentsToColor = (comps, scale = 1) => comps?.length === 1
    ? grayscale(comps[0] * scale)
    : comps?.length === 3
        ? rgb(comps[0] * scale, comps[1] * scale, comps[2] * scale)
        : comps?.length === 4
            ? cmyk(comps[0] * scale, comps[1] * scale, comps[2] * scale, comps[3] * scale)
            : undefined;
// prettier-ignore
export const colorToComponents = (color) => color.type === ColorTypes.Grayscale
    ? [color.gray]
    : color.type === ColorTypes.RGB
