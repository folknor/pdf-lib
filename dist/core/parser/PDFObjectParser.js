...
class PDFObjectParser extends BaseParser {
    static forBytes = (bytes, context, capNumbers) => new PDFObjectParser(ByteStream.of(bytes), context, capNumbers);
    static forByteStream = (byteStream, context, capNumbers = false) => new PDFObjectParser(byteStream, context, capNumbers);
    context;
    cryptoFactory;
    ...
    matchPrefixedKeyword(keyword) {
        ...
        if (currentOffset > 0) {
            ...
            const isValidPrefix = IsWhitespace[prevByte] ||
