...
function setupSyllables(_font, glyphs) {
    let syllable = 0;
    let last = 0;
    for (const [start, end, tags] of stateMachine.match(glyphs.map(indicCategory))) {
        if (start > last) {
            ++syllable;
...
function consonantPosition(_font, consonant, virama) {
    ...
    if (wouldSubstitute(glyphs.slice(0, 2), 'blwf') ||
        wouldSubstitute(glyphs.slice(1, 3), 'blwf')) {
        return POSITIONS.Below_C;
    }
    else if (wouldSubstitute(glyphs.slice(0, 2), 'pstf') ||
        wouldSubstitute(glyphs.slice(1, 3), 'pstf')) {
        return POSITIONS.Post_C;
    }
    else if (wouldSubstitute(glyphs.slice(0, 2), 'pref') ||
...
function initialReordering(font, glyphs, plan) {
    ...
    for (let start = 0, end = nextSyllable(glyphs, 0); start < glyphs.length; start = end, end = nextSyllable(glyphs, start)) {
        const { category: _category, syllableType } = glyphs[start].shaperInfo;
        ...
        if (indicConfig.rephPos !== POSITIONS.Ra_To_Become_Reph &&
            ...
            if (wouldSubstitute(g.slice(0, 2), 'rphf') ||
        ...
        else if (indicConfig.rephMode === 'Log_Repha' &&
    ...
    for (let start = 0, end = nextSyllable(glyphs, 0); start < glyphs.length; start = end, end = nextSyllable(glyphs, start)) {
        ...
        if (start + 1 < end && start < base) {
            ...
            if (plan.unicodeScript !== 'Malayalam' &&
                ...
                if (isHalantOrCoeng(glyphs[newPos]) &&
        ...
        if (glyphs[start].shaperInfo.position === POSITIONS.Pre_M &&
            (!start || !/Cf|Mn/.test(getCategory(glyphs[start - 1].codePoints[0])))) {
...
//# sourceMappingURL=IndicShaper.js.map