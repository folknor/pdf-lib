            ...
            case 4: {
                ...
                for (const ligature of table.ligatureSets.get(index)) {
                    ...
                    for (const matchedIndex of matched) {
                        characters.push(...this.glyphs[matchedIndex].codePoints);
                    }
                    // Create the replacement ligature glyph
                    const ligatureGlyph = new GlyphInfo(this.font, ligature.glyph, characters, curGlyph.features);
                    ...
                    for (const matchIndex of matched) {
                        ...
                        if (isMarkLigature) {
                            ...
                            while (idx < matchIndex) {
                                const ligComponent = curComps -
                                    lastNumComps +
                                    Math.min(this.glyphs[idx].ligatureComponent || 1, lastNumComps);
                                this.glyphs[idx].ligatureID = ligatureGlyph.ligatureID;
                                this.glyphs[idx].ligatureComponent = ligComponent;
                                idx++;
                            }
                        }
...
//# sourceMappingURL=GSUBProcessor.js.map