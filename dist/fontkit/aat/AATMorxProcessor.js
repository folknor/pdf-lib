...
export default class AATMorxProcessor {
    ...
    processContextualSubstitution(glyph, entry, index) {
        ...
        if (entry.currentIndex !== 0xffff) {
            ...
            const gid2 = lookupTable.lookup(glyph.id);
            if (gid2) {
                this.glyphs[index] = this.font.getGlyph(gid2, glyph.codePoints);
            }
        }
    ...
    processNoncontextualSubstitutions(subtable, glyphs, _index) {
        ...
        for (let i = 0; i < glyphs.length; i++) {
            ...
            if (glyph.id !== 0xffff) {
                ...
                if (gid) {
                    // 0 means do nothing
                    glyphs[i] = this.font.getGlyph(gid, glyph.codePoints);
                }
            }
        }
    }
    _insertGlyphs(glyphIndex, insertionActionIndex, count, isBefore) {
        const insertions = [];
        while (count--) {
            const gid = this.subtable.table.insertionActions.getItem(insertionActionIndex++);
    ...
    generateInputsForSubtable(subtable) {
        ...
        stateMachine.traverse({
            enter: (glyph, entry) => {
                ...
                if (count === 1) {
                    const result = input.map((item) => item.id);
                    const cached = this.inputCache[found];
                    if (cached) {
                        cached.push(result);
                    }
                    else {
...
//# sourceMappingURL=AATMorxProcessor.js.map